...
window.CalculUpCore = (function() {
    ...
    let isLoading = false;
    let demoMode = false;  // Mode démonstration sans Firebase
...
case 'home':
    ...
    break;
            ...
            case 'demo-login':
                if (window.CalculUpDemo) {
                    CalculUpDemo.showDemoLoginScreen();
                } else {
                    showError('Mode démo non disponible');
                    navigateToScreen('login');
                }
                break;
    ...
    function setDemoMode(enabled) {
        demoMode = enabled;
        console.log('🎭 Mode démo:', enabled ? 'activé' : 'désactivé');
    }

    function isDemoMode() {
        return demoMode || (window.CalculUpDemo && CalculUpDemo.isDemoModeActive());
    }

    function setDemoUser(demoUser) {
        user = demoUser;
        console.log('🎭 Utilisateur démo défini:', demoUser?.identifier);
    }
    ...
    return {
        ...
        getUser: () => {
            ...
            if (isDemoMode() && window.CalculUpDemo) {
                return CalculUpDemo.getCurrentDemoUser() || user;
            }
            return user;
        },
        isLoggedIn: () => {
            if (isDemoMode() && window.CalculUpDemo) {
                return CalculUpDemo.isDemoModeActive();
            }
            return !!user;
        },
...
console.log('✅ Module CalculUpCore chargé avec mode démo et signalements');